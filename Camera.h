...
public:
	...
	void ProcessKeyboard(Camera_Movement direction, GLfloat deltaTime) {
		...
		if (direction == RIGHT)
			this->Position += this->Right * velocity;
		this->Position.y = 0;
		if (direction == JUMP && jumped == false) {
			jumpTime = glfwGetTime();
			jumped = true;
		} else {
			GLfloat airTime = glfwGetTime() - jumpTime;
			this->Position.y = (GLfloat) velocity * airTime * 100
					- 1.0 / 2 * 9.8 * airTime * airTime;
		...
		if (direction == FALL) {
			GLfloat height = this->Position.y;
			GLfloat airTime = glfwGetTime() - jumpTime;
			this->Position.y = (GLfloat) velocity * 100 * airTime
					- 1.0 / 2 * 9.8 * airTime * airTime;
			if (this->Position.y < 0) {
				this->Position.y = 0;
			}
